    ...
    switch ($functionName) {
        ...
        <div class="panel">
            ...
            <div class="card">
                ...
                <?php 
                $salesPreview = getSalesByPeriod($conn, date('Y-m-01'), date('Y-m-d'));
                $totalSales = getTotalSalesByPeriod($conn, date('Y-m-01'), date('Y-m-d'));
                ?>
                <div class="preview-stats">
                    <div>Текущий месяц: <?php echo number_format($totalSales, 2); ?> руб.</div>
                    <div>Записей: <?php echo $salesPreview->num_rows; ?></div>
                </div>
                <form method="POST">
            ...
            <div class="card">
                ...
                <?php 
                $reviewsPreview = getProductReviews($conn);
                ?>
                <div class="preview-stats">
                    <div>Последних отзывов: <?php echo $reviewsPreview->num_rows; ?></div>
                    <div>Новых сегодня: <?php 
                        $todayReviews = $conn->query("SELECT COUNT(*) FROM reviews WHERE DATE(reviews_date) = CURDATE()")->fetch_row()[0];
                        echo $todayReviews;
                    ?></div>
                </div>
                <form method="POST">
            ...
            <div class="card">
                ...
                <?php 
                $cartPreview = getCartContents($conn);
                $totalInCarts = $conn->query("SELECT SUM(quantity) FROM cart_items")->fetch_row()[0];
                ?>
                <div class="preview-stats">
                    <div>Товаров в корзинах: <?php echo $totalInCarts; ?></div>
                    <div>Пользователей: <?php echo $cartPreview->num_rows; ?></div>
                </div>
                <form method="POST">
            ...
            <div class="card">
                ...
                <?php 
                $avgOrders = getAverageOrdersPerCustomer($conn);
                $totalCustomers = $conn->query("SELECT COUNT(*) FROM users WHERE status = 'user'")->fetch_row()[0];
                ?>
                <div class="preview-stats">
                    <div>Среднее: <?php echo round($avgOrders, 2); ?></div>
                    <div>Клиентов: <?php echo $totalCustomers; ?></div>
                </div>
                <form method="POST">
            ...
            <div class="card">
                ...
                $topProduct = $topProducts->fetch_assoc();
                ?>
                <div class="preview-stats">
                    <div>Лидер: <?php echo htmlspecialchars($topProduct['pname'] ?? 'Нет данных'); ?></div>
                    <div>Выручка: <?php echo number_format($topProduct['total_revenue'] ?? 0, 2); ?> руб.</div>
                </div>
                <form method="POST">
            ...
            <div class="card">
                ...
                <?php 
                $customersByCity = getCustomersByCity($conn);
                $topCity = $customersByCity->fetch_assoc();
                ?>
                <div class="preview-stats">
                    <div>Всего городов: <?php echo $customersByCity->num_rows; ?></div>
                    <div>Топ: <?php echo htmlspecialchars($topCity['city'] ?? 'Нет данных'); ?></div>
                </div>
                <form method="POST">
            ...
            <div class="card">
                ...
                <?php 
                $monthSales = getTotalSalesByPeriod($conn, date('Y-m-01'), date('Y-m-d'));
                $weekSales = getTotalSalesByPeriod($conn, date('Y-m-d', strtotime('-7 days')), date('Y-m-d'));
                ?>
                <div class="preview-stats">
                    <div>За месяц: <?php echo number_format($monthSales, 2); ?> руб.</div>
                    <div>За неделю: <?php echo number_format($weekSales, 2); ?> руб.</div>
                </div>
                <form method="POST">
            ...
            <div class="card">
                ...
                $topCustomer = $topCustomers->fetch_assoc();
                ?>
                <div class="preview-stats">
                    <div>Топ клиент: <?php echo htmlspecialchars($topCustomer['firstname'] ?? 'Нет данных'); ?></div>
                    <div>Потратил: <?php echo number_format($topCustomer['total_spent'] ?? 0, 2); ?> руб.</div>
                </div>
                <form method="POST">
            ...
            <div class="card">
                ...
                <div class="preview-stats">
                    <div>Введите ID клиента</div>
                    <div>для просмотра истории</div>
                </div>
                <form method="POST">
            ...
            <div class="card">
                ...
                <?php 
                $lowStock = getLowStockProducts($conn, 10);
                $lowStockCount = $lowStock->num_rows;
                ?>
                <div class="preview-stats">
                    <div>Товаров мало: <?php echo $lowStockCount; ?></div>
                    <div>Порог: 10 шт.</div>
                </div>
                <form method="POST">
            ...
            <div class="card">
                ...
                <div class="preview-stats">
                    <div>Введите ID клиента</div>
                    <div>для просмотра статистики</div>
                </div>
                <form method="POST">
            ...
            <div class="card">
                ...
                <?php 
                $suppliers = getSuppliersWithProducts($conn);
                $suppliersCount = $suppliers->num_rows;
                ?>
                <div class="preview-stats">
                    <div>Поставщиков: <?php echo $suppliersCount; ?></div>
                    <div>Товаров: <?php 
            ...
            <?php if ($result): ?>
                ...
                <?php elseif ($functionName === 'total_sales'): ?>
                    <div class="stat-card">
                        ...
                        <div class="stat-value">$<?php echo number_format($result, 2); ?></div>
                    </div>
